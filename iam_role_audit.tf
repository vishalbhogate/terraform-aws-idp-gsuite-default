...
data aws_iam_policy_document "audity_policy" {
  statement {
...
resource aws_iam_role_policy "audity_assume" {
  name   = "assume-idp-audit"
  role   = aws_iam_role.audit.id
  policy = data.aws_iam_policy_document.audity_policy.json
}
...
resource aws_iam_group_policy audit_assume_group {
